\begin{abstract}
    ...
    blowing up the condition number of the projection correlation matrix beyond $10^{10}$, % I can produce runs where the inversion just fails. From the test I made, for a simple hn with not too many chunks, I got the hn to just fail to learn with early stops.
    causing catastrophic inversion failures that the standard regularization
    ...
    completely resolves matrix inversion collapse, and that \texttt{iFOPNG} consistently
...
% Maybe I should cut the linear algebra and leave: Traditional projection methods collapse under hypernetwork architectures. The architectural necessity of weight-chunking forces the model to accumulate an exceptionally large gradient volume before a single projection step can execute, causing catastrophic numerical singularities that standard regularization cannot rescue
% Maybe leave the algebra.