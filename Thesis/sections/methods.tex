...
Following \citet{farquhar_towards_2018}, who demonstrated that multi-head
...
\end{equation}
estimated over $N$ held-out samples.
...
ONG \citep{yadav_ong_2025} applies OGD-style projection to the natural gradient rather than to the
...
\begin{equation}
    ...
    = -\eta\!\left(
      ...
      \right).
\end{equation}
Unlike FOPNG, the projection is measured in Euclidean rather than Fisher metric space.

%dully note:
Following the diagonal Fisher approximation used throughout this thesis, we substitute the EKFAC inverse of \citet{yadav_ong_2025} with the diagonal empirical Fisher; 
results are not directly comparable to the original implementation.
...
\begin{equation}
    ...
    \qquad
    P = I - \hat{F}_{\mathrm{old}}\,G\!\left(G^\top\hat{F}_{\mathrm{old}}\,
        \hat{F}^{-1}\hat{F}_{\mathrm{old}}\,G
        + \lambda I\right)^{\!-1}\!G^\top\hat{F}_{\mathrm{old}},
    \label{eq:fopng}
\end{equation}
where $\hat{F} \equiv \hat{F}_{\mathrm{new}}$ is the current-task Fisher,
$\hat{F}_{\mathrm{old}}$ is the Fisher under which the stored directions
were collected, and $\lambda$ is added to the diagonal for numerical
stability.
This is the pre-Fisher form of the practical algorithm of
\citet{garg_fisher-orthogonal_2026}, in which the stored basis carries its
collection-time Fisher weighting; it is the variant implemented and
evaluated throughout this thesis, and it makes the relationship to
\texttt{iFOPNG} exact: the latter is obtained by the single substitution
$\hat{F}_{\mathrm{new}} \to F_c$ in the preconditioner, the trust region,
...
\begin{enumerate}
    ...
    \item \textbf{Metric-tensor scaling.}
        ...
        \begin{equation}
            s = \max\bigl(\max_i(F^*)_i,\; 1\bigr),
            \qquad
            \tilde{F} = F \,/\, s
            \;\;\text{for } F \in \bigl\{\hat{F}_{\mathrm{old}},\,
            \hat{F}_{\mathrm{new}},\, F^*\bigr\},
            \qquad \tilde{\lambda} = \lambda \,/\, s,
            \label{eq:scaling}
        \end{equation}
        where $F^* = \hat{F}_{\mathrm{new}}$ for \texttt{FNG}, \texttt{ONG},
