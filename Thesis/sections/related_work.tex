...
% Failure to maintain stability under this tension is the catastrophic
% forgetting introduced in Section~\ref{sec:introduction}; the remainder of this
% section surveys the algorithmic responses to it.
...
% This metric is invariant under smooth reparameterization, making
% Fisher-norm constraints more principled than Euclidean-norm constraints
% for models whose parameters are not isotropically scaled.
...
% hardness: where EWC applies a soft penalty that can be overwhelmed by
% sufficiently large task gradients, projection methods impose a geometric
% constraint that is either satisfied or not.
