...
from gradient import AVECollector, GradientMemory
...
class FOPNG:
    def __init__(
        ...
        self.GradientCollector = AVECollector(grads_per_task)
    ...
    def after_task(self, model: nn.Module, task_id, loader: DataLoader, criterion: Callable) -> None:
        ...
        if self.debug:
            print(f"[FOPNG] Current G memory size: {len(self.gradient_memory)} / {self.max_directions}")
        ...
        logs = {
            ...
            "fopng/memory/G_cols": len(self.gradient_memory),
    ...
    for t, loader in enumerate(train_loaders):
        ...
        if t == 0:
            if not saved:
                ...
                opt = first_task_optimizer_cls(model.parameters(), lr=1e-3)
                while best_loss >= loss_to_achieve and loss_repeat < 10 and epoch < _max_epochs:
                    ...
                    if lr_patience_counter >= 5:
                        ...
                        if verbose: print(f"    [Scheduler] Loss stalled. Lowering LR to {fopng.lr}")
                    epoch += 1
            ...
            while best_loss >= loss_to_achieve and loss_repeat < 10 and epoch < _max_epochs:
                F_new = fopng.FisherEstimator.estimate(model, task_id, loader, criterion, device)
                ...
                if verbose: print(f"  epoch {epoch+1}/{_max_epochs} loss={avg_loss:.4f} w_rho={avg_weighted_rho:.4f} corr={avg_correction_norm:.4e} raw_rho={avg_raw_rho:.4f}, lr={fopng.lr}")
                epoch += 1
