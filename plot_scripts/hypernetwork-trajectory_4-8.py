"""
hypernetwork-trajectory_4-8.py
────────────────────────────────
Trajectory + bar chart: Exp 4 (Split-CIFAR10 MH Standalone) vs
Exp 8 (Split-CIFAR10 HN Standard). Sub-RQ1 CIFAR10 comparison.
Fetches data from W&B.

Outputs:
  plots/hypernetwork-trajectory_4-8.pdf
  plots/hypernetwork-trajectory_4-8-bars.pdf
"""
import os
import numpy as np
import pandas as pd
...
PANEL_META = {
    404: "(a) Multihead CNN — Standalone",
...
def plot_barchart(df, save_path):
    ...
    agg = (df.groupby(["experiment_id", "method"])
             .agg(acc_mean=("final_avg_acc", "mean"),
                  ...
                  bwt_mean=("bwt", "mean"))
             .reset_index())
    hn_order  = (agg[agg.experiment_id == 408]
                 .sort_values("acc_mean", ascending=False)["method"].tolist())
    extra     = [m for m in agg[agg.experiment_id == 404]["method"].tolist()
    ...
    for eid, offset, hatch, alpha, label in [
        (404, -bw/2, "///", 0.45, "Standalone MH CNN (Exp 4)"),
        (408,  bw/2, "",   0.85, "Standard HN (Exp 8)"),
    ]:
