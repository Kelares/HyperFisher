...
def train():
    ...
    RUN_DIR = f"runs/{CURRENT_CONFIG.model.value}_{CURRENT_CONFIG.level.value}_Loss_{LOSS_ACHIEVED}"
    PATH_OF_SAVE = f"{RUN_DIR}/agent.pt"
    actor.load_state_dict(torch.load(PATH_OF_SAVE, map_location=DEVICE))

    best_save_path = Path(f"runs/{CURRENT_CONFIG.model.value}_best.pt")
