...
# --- DATA PREPARATION ---
def loadDataset(CurrentConfig):
    print(f"Loading Minari Dataset: {CurrentConfig.dataset_id} (downloading if needed)...")
    # "Medium" is best for training (contains mix of success/fail)
    md = minari.load_dataset(CurrentConfig.dataset_id, download=True)
    ...
    np.savez(f"{MODULE_DIR}/normalizations/{CurrentConfig.level.value}.npz", mean=mean, std=std)
...
def liveEnv(CurrentConfig, DEVICE, loss):
    ...
    stats = np.load(f"{MODULE_DIR}/normalizations/{CurrentConfig.level.value}.npz")
