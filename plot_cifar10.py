...
def plotJob(job_type):
    # 2. Fetch runs
    runs = api.runs(f"{entity}/{project}", filters={"jobType": job_type, "group": "split_cifar10", "state": "finished"})
    ...
    df = pd.DataFrame(data_list)
                # # 1. Filter for EFOPNG
                # efopng_df = df[df["Method"] == "EFOPNG"]

                # # 2. Sort by Task and Seed for easier reading
                # efopng_df = efopng_df.sort_values(by=["Tasks Trained (t)", "Seed"])

                # # 3. Display all rows for manual check
                # print(efopng_df)

                # # 4. Specifically look at t=1 to find the source of the 0.8 on the chart
                # print("\n--- Accuracy at Task 1 (t=1) ---")
                # print(efopng_df[efopng_df["Tasks Trained (t)"] == 1])

                # # 5. Check the mean at t=1 (this is what the chart plots)
                # print(f"\nMean at t=1: {efopng_df[efopng_df['Tasks Trained (t)'] == 1]['Average Accuracy'].mean()}")
    ...
    # Formatting
    plt.title("Continual Learning Performance: Average Accuracy Trajectory", fontsize=15, fontweight='bold')
    ...
    # Save for Thesis
    name = f"average_accuracy_trajectory_{job_type}"
